...
BadGzipFile = getattr(gzip, 'BadGzipFile', OSError)
...
def read_events_table_by_row(mimic3_path, table):
    ...
    #nb_rows = {'labevents': 122103667}
    #csv_files = {'labevents': 'hosp/labevents.csv.gz'}
    ...
    file_path = os.path.join(mimic3_path, csv_files[table.lower()])
    # with gzip.open(file_path, mode='rt', encoding='utf-8', errors='replace') as f:
        ...
        try:
            with gzip.open(file_path, mode='rt', encoding='utf-8', errors='replace') as f:
                ...
                for row in reader:
                    ...
                    i += 1
            break  # normal EOF
        except (BadGzipFile, OSError) as e:
            logging.warning(f"Skipping corrupted gzip member at row {idx}: {e!r}")
